...
class TestTree(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # root
        n0 = tree.TreeNode(None, None, [], {'name': '0'})

        # 1st level
        n00 = tree.TreeNode(n0, 0, None, {'name': '00'})  # leaf
        n0.children.append(n00)
        n01 = tree.TreeNode(n0, 1, [], {'name': '01'})
        n0.children.append(n01)
        n02 = tree.TreeNode(n0, 2, [], {'name': '02'})
        n0.children.append(n02)

        # 2nd level
        n010 = tree.TreeNode(n01, 0, None, {'name': '010'})  # leaf
        n01.children.append(n010)
        n011 = tree.TreeNode(n01, 1, [], {'name': '011'})
        n01.children.append(n011)
        n020 = tree.TreeNode(n02, 0, [], {'name': '020'})
        n02.children.append(n020)

        # 3rd level
        n0110 = tree.TreeNode(n011, 0, None, {'name': '0110'})  # leaf
        n011.children.append(n0110)
        n0200 = tree.TreeNode(n020, 0, None, {'name': '0200'})  # leaf
        n020.children.append(n0200)
        n0201 = tree.TreeNode(n020, 1, None, {'name': '0201'})  # leaf
        n020.children.append(n0201)
        n0202 = tree.TreeNode(n020, 2, None, {'name': '0202'})  # leaf
    ...
    def test_get_nodes_dfs(self):
        with self.subTest('from_start=True, predicate=None'):
            nodes = self.test_tree.get_nodes_dfs(from_start=True,
                                                 predicate=None)
            nodes_data = list(map(lambda node: node.data['name'], nodes))
